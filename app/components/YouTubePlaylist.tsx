...
import type { PlaylistItem } from './YouTubePlayer';
...
export default function YouTubePlaylist({ player,
  playlist, currentClipIndex, setCurrentClipIndex }: {
    player: any;
    playlist: Playlist;
...

                  {
                    playlist.map((clip, index) => {
                      return <button key={index}
                        ...
                        <span className={`inline-block w-2 h-2 ml-px mr-4 rounded-full ${currentClipIndex === index ? 'bg-pink-500' : 'bg-gray-500'}`}></span>
                        <div>
                          <h2 className={`${currentClipIndex === index ? "text-pink-500" : "text-gray-200"}`}
                          >Example of task change</h2>
                          <div className="flex items-center text-xs text-gray-400">
                            ...
                            <p>{calculateDuration(clip.attributes.start, clip.attributes.end)}</p>
                          </div>
                        </div>
                      </button>
                    })
